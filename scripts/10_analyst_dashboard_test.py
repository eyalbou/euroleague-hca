...
self_context = [
    {
        "framing": "Raw points per game",
        "value": f"{league_hca:+.2f}",
        "unit": "pts",
        "ci": f"95% CI [{league_hca_lo:+.2f}, {league_hca_hi:+.2f}]",
        "note": "Mean home margin across all games. The headline number.",
    },
    {
        "framing": "% of total game scoring",
        "value": f"{league_hca / total_pts_mean * 100:.2f}",
        "unit": "%",
        "ci": f"avg total {total_pts_mean:.0f} pts/game",
        "note": "Pace-agnostic. Useful for comparing to leagues with very different scoring (e.g. NBA ~230 pts/game).",
    },
    {
        "framing": "Per 100 possessions",
        "value": f"{hca_per_100:+.2f}",
        "unit": "net rtg",
        "ci": f"measured pace = {EUROLEAGUE_PACE_MEASURED:.1f} poss/team",
        "note": "Pace-adjusted (net-rating differential). Standard basketball-analytics framing.",
    },
    {
        "framing": "Home win rate above 50%",
        "value": f"+{(home_win_rate - 0.5) * 100:.1f}",
        "unit": "pp",
        "ci": f"raw rate {home_win_rate * 100:.1f}%",
        "note": "Bounded 0-50. The crudest but most universal HCA metric.",
    },
]
...
<div class="app">
  ...
  <section class="bench">
    <div class="bench-title">For context &middot; the same EuroLeague HCA, expressed four ways</div>
    <p class="bench-sub">Cross-league benchmarks were removed: NBA / NCAA / EuroCup numbers we did not freshly measure
      here would be misleading next to a measured EuroLeague row. Instead, here is our number framed in the four units
      analysts most commonly ask for. All values come from the same dataset.</p>
    ...
    <p class="bench-foot">Note: per-100-possessions uses measured EuroLeague pace of ~{EUROLEAGUE_PACE_MEASURED:.1f} possessions per team per game
      (FIBA-rules average over 2015-2024; box-score possession counts are not in this layer). To restore a true
      cross-league comparison, ingest live EuroCup (same API), NBA (`nba_api`), or NCAA (KenPom) and re-measure here.</p>
  </section>
  ...
  <section class="panel" id="tab-verdict">
    ...
    <div class="gap-card">
      ...
      <ul>
        <li><code>Shooting splits</code> -- home vs away FG%, 3P%, FT%. The classic HCA mechanism question. Requires box-score
          aggregates (available in the API but not yet ingested).</li>
        <li><code>Foul / FT differential</code> -- the strongest published mechanism for HCA in basketball (referee bias,
          Moskowitz &amp; Wertheim 2011). Same data path as shooting splits.</li>
...
(() => {
  ...
  P.self_context.forEach(r => {
    ...
    tr.classList.add('ours');
    tr.innerHTML = `
      <td>${r.framing}</td>
      <td class="num"><strong>${r.value}</strong> <span style="color:var(--fg-mute);font-weight:400;">${r.unit}</span></td>
